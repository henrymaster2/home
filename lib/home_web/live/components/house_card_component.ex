  ...
  def render(assigns) do
    ...
    >
      <%= for house <- @houses do %>
        <div
          data-house={house.id}
          class="absolute inset-0 flex flex-col justify-between opacity-0 transition-opacity duration-700 ease-in-out"
        >
            ...
            <div class="flex gap-1.5 w-full pt-1">
              <%= for {_, index} <- Enum.with_index(house.images) do %>
                <div class="h-1 flex-1 bg-white/20 rounded-full overflow-hidden backdrop-blur-md">
                  <div
                    data-house-progress={index}
                    class="h-full bg-amber-400 shadow-[0_0_8px_#fbbf24]"
                    style="width: 0%;"
                  >
                  </div>
                </div>
