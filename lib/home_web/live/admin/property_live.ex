defmodule HomeWeb.Admin.PropertyLive do
  ...
  defp clear_expired_statuses do
    ...
    Property
    |> where([p], p.status != "Available" and not is_nil(p.unavailable_until) and p.unavailable_until <= ^now)
  ...
  @impl true
  def handle_event("update_prices", %{"id" => id_str, "night_price" => night_price, "day_price" => day_price}, socket) do
    id = String.to_integer(id_str)
    property = Repo.get!(Property, id)

    changeset = cast(property, %{"night_price" => night_price, "day_price" => day_price}, [:night_price, :day_price])

    case Repo.update(changeset) do
      {:ok, _} ->
        {:noreply, socket |> put_flash(:info, "Pricing updated successfully.") |> fetch_properties()}
      {:error, _} ->
  ...
  def handle_event("delete_property", %{"id" => id_str}, socket) do
    ...
    case Repo.transaction(fn ->
      ...
      {:ok, _} ->
        {:noreply, socket |> put_flash(:info, "Property successfully removed.") |> fetch_properties()}
      {:error, _reason} ->
  ...
  def render(assigns) do
    ...
    <div class="flex flex-col lg:flex-row min-h-screen bg-zinc-950 text-zinc-100 font-sans antialiased selection:bg-blue-600/30 selection:text-blue-200">
      ...
      <header class="bg-zinc-900/90 backdrop-blur-md border-b border-zinc-800/80 px-6 py-4 flex lg:hidden justify-between items-center sticky top-0 z-40">
        ...
        <button phx-click="toggle_mobile_menu" class="px-3.5 py-2 bg-zinc-950 rounded-xl border border-zinc-800 text-zinc-300 hover:text-white hover:border-zinc-700 transition-all text-xs font-semibold tracking-wide">
        ...
        <div>
          <div class="hidden lg:flex p-6 border-b border-zinc-800/40">
            <h1 class="text-lg font-extrabold text-white flex items-center gap-2.5 tracking-tight">
              <span class="bg-gradient-to-br from-blue-500 to-indigo-600 text-white p-2 rounded-xl shadow-md shadow-blue-500/20 text-xs">🏠</span>
          ...
          <nav class="p-4 space-y-1">
            ...
            <div role="button" title="Coming Soon"
              ...
              <span class="bg-zinc-900 border border-zinc-800 text-zinc-500 font-extrabold px-2 py-0.5 rounded text-[9px] uppercase tracking-normal">Soon</span>
            </div>
          </nav>
        </div>
      </aside>
      ...
      <main class="flex-1 flex flex-col lg:h-screen lg:overflow-y-auto">
        <header class="bg-zinc-900/40 border-b border-zinc-800/60 px-8 py-5 hidden lg:flex justify-between items-center backdrop-blur-sm">
          <h2 class="text-xs font-bold uppercase tracking-widest text-zinc-400">Property Management Overview</h2>
        ...
        <div class="p-4 md:p-8 max-w-7xl w-full mx-auto space-y-6">
              ...
              <div class="bg-zinc-900/30 border border-zinc-800/70 hover:border-zinc-700/80 rounded-2xl overflow-hidden flex flex-col justify-between shadow-xl backdrop-blur-md transition-all duration-300 group hover:shadow-2xl hover:shadow-blue-950/5">
                <div>
                      ...
                      <div class="flex overflow-x-auto snap-x snap-mandatory h-full no-scrollbar scroll-smooth">
                        <%= for img <- property.property_images do %>
                          <div class="w-full h-full flex-shrink-0 snap-center relative">
                            <img src={img.image_url} class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.01]" loading="lazy" />
                          </div>
                        <% end %>
                      </div>
                  ...
                  <div class="p-5 space-y-4">
                    <div>
                      <h4 class="text-base font-bold text-white tracking-tight group-hover:text-blue-400 transition-colors truncate"><%= property.title %></h4>
                      <p class="text-xs text-zinc-400 flex items-center gap-1.5 mt-1">
                        <span class="text-zinc-500">📍</span> <%= property.location %> • <span class="text-blue-400 font-semibold uppercase tracking-wider text-[10px]"><%= property.tier %></span>
                      </p>
                    </div>
                    
                    <% # Feature Pills%>
                    <div class="flex flex-wrap gap-1.5">
                      <%= if property.wifi do %><span class="text-[9px] bg-zinc-950/80 px-2.5 py-1 rounded-md border border-zinc-800/80 text-zinc-400 font-bold tracking-wide uppercase">📶 WiFi</span><% end %>
                      <%= if property.tv do %><span class="text-[9px] bg-zinc-950/80 px-2.5 py-1 rounded-md border border-zinc-800/80 text-zinc-400 font-bold tracking-wide uppercase">📺 Smart TV</span><% end %>
                      <%= if property.music_system do %><span class="text-[9px] bg-zinc-950/80 px-2.5 py-1 rounded-md border border-zinc-800/80 text-zinc-400 font-bold tracking-wide uppercase">🔊 Sound</span><% end %>
                    </div>
    
                    <% # Dual Price Rate Modification Interface %>
                    <div class="pt-4 border-t border-zinc-800/40">
                      <label class="text-[10px] font-bold tracking-widest text-zinc-400 uppercase block mb-2">Edit Rates (KES)</label>
                      <form phx-submit="update_prices" class="grid grid-cols-2 gap-2">
                        ...
                        <div class="bg-zinc-950 p-2 rounded-xl border border-zinc-800 focus-within:border-blue-500/50 transition-all">
                          <span class="block text-[8px] uppercase font-bold text-zinc-500 tracking-wider">🌙 Night Price</span>
                          <input 
                            type="number" 
                            name="night_price" 
                        ...
                        <div class="bg-zinc-950 p-2 rounded-xl border border-zinc-800 focus-within:border-blue-500/50 transition-all">
                          <span class="block text-[8px] uppercase font-bold text-zinc-500 tracking-wider">☀️ Day Price</span>
                          <input 
                            type="number" 
                            name="day_price" 
                        ...
                        <button type="submit" class="col-span-2 text-[9px] font-bold uppercase tracking-wider bg-zinc-800 hover:bg-zinc-700 text-zinc-300 py-1.5 rounded-lg border border-zinc-700/50 transition-all">
                    ...
                    <%= if property.status != "Available" and not is_nil(property.unavailable_until) do %>
                      <div class="text-[11px] font-medium text-amber-400 bg-amber-500/5 border border-amber-500/10 px-3 py-2 rounded-xl flex items-center gap-2 shadow-inner">
                        ...
                        <span>Locked Until: <strong class="text-zinc-200 font-semibold"><%= format_datetime(property.unavailable_until) %></strong></span>
                      </div>
                    <% end %>

                    <% # Status Modifiers %>
                    <div class="pt-4 border-t border-zinc-800/40">
                      <label class="text-[10px] font-bold tracking-widest text-zinc-500 uppercase block mb-2">Availability Settings</label>
                      
                      <form phx-submit="update_status" class="space-y-2.5">
                        <input type="hidden" name="id" value={property.id} />
                        
                        <div class="relative">
                          <select name="status" class="w-full bg-zinc-950 border border-zinc-800 hover:border-zinc-700 text-zinc-200 text-xs font-semibold rounded-xl px-3 py-2.5 outline-none transition-all focus:ring-1 focus:ring-blue-500/50 focus:border-blue-500 cursor-pointer appearance-none">
                            <option value="Available" selected={property.status == "Available"}>🟢 Available</option>
                            <option value="Occupied" selected={property.status == "Occupied"}>🔴 Occupied / Booked</option>
                            <option value="Maintenance" selected={property.status == "Maintenance"}>🛠️ Maintenance Lock</option>
                          </select>
                        ...
                        <div class="space-y-1">
                          <span class="text-[9px] font-bold text-zinc-400 tracking-wide uppercase block">Available From (Optional)</span>
                          <input 
                            type="datetime-local" 
                        ...
                        <button type="submit" class="w-full bg-zinc-900 border border-zinc-800 hover:border-blue-500/40 hover:bg-blue-600 hover:text-white text-zinc-300 text-[10px] font-bold tracking-widest uppercase py-2 px-3 rounded-xl transition-all duration-200 shadow-md">
          ...
          <%= if @properties == [] do %>
            <div class="text-center py-20 bg-zinc-900/10 border border-dashed border-zinc-800 rounded-3xl max-w-md mx-auto mt-12">
              ...
              <h3 class="text-sm font-bold text-zinc-200 uppercase tracking-wide">No Properties Match</h3>
              <p class="text-xs text-zinc-500 mt-1.5 max-w-xs mx-auto leading-relaxed">No real estate assets match your search terms. Try refining your query or add a brand new item.</p>
              <.link navigate={~p"/admin"} class="inline-block mt-5 text-xs font-bold uppercase tracking-wider bg-zinc-900 hover:bg-zinc-800 text-blue-400 px-5 py-2.5 rounded-xl transition-all border border-zinc-800 shadow-lg">
                Create First Unit
              </.link>
            </div>
          <% end %>
  ...
  defp status_color_classes("Maintenance"), do: "bg-amber-500/5 border-amber-500/20 text-amber-400"
